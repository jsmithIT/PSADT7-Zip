...
Try {
	...
	[string]$appVersion = '22.1.0.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '5.3.4'
	[string]$appScriptDate = '2022.08.10'
	...
	$CloseApps = "7zFM=7-Zip Management Console, 7zG=7-Zip"
	...
	$64bitAppInstallPath = "$PSScriptRoot\Files\7z2201-x64.exe"
	# Application  install parameters.
	$64bitAppInstallParam = "/S" 
	...
	$64bitAppUninstallPath = "$Env:Programfiles\7-Zip\Uninstall.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/S"
	...
	# Directory of where settings for application will be put.
    $appSettingsDir = "$Env:ProgramData\Tuolumne County\Application Settings\7-Zip"

	# Name of activesetup key
	$keyName = "7-Zip - Default Settings"

	# Activesetup key path
	$keyPath = "HKLM:\SOFTWARE\Microsoft\Active Setup\Installed Components\$keyName"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		# Else, log results from check.
			# Check if previous EXE versions of application are installed.  
			If (Test-Path -Path "$Env:ProgramFiles\7-Zip\Uninstall.exe") {
			...
			ElseIf (Test-Path -Path "$Env:ProgramFiles\7-Zip\") {
				
			}
			Else {
				Write-Log -Message "Previous versions of $apName do not exist."
			}

		# Check if previous versions of activesetup key exist.
		# Remove the activesetup key.
		# Create scriptblock of code to be run for registry changes.
		# Run the HKCU scriptblock.
		Write-Log -Message "Checking if previous versions of activesetup key exist." 
        If (Test-Path -Path "$keyPath") {
		# Remove previous versions of activesetup key. 
		Try {
			Write-Log -Message "Removing previous versions of activesetup key."
            Set-ActiveSetup -Key "$keyName" -PurgeActiveSetupKey
			}
		...
		Write-Log -Message "Checking if previous versions of settings directory exist." 
		If (Test-Path -Path "$appSettingsDir") {
			Write-Log -Message "Previous versions of settings directory exist."  
			# Remove previous versions of settings directory.
			Try {
				Write-Log -Message "Removing previous versions of settings directory." 
			...
			Write-Log -Message "Removing previous versions of settings directory complete."
		}
		# Else, log results from check. 
		Else {  
        ...
        Try {
            ...
            Copy-File -Path "$PSScriptRoot\SupportFiles\7-Zip" -Destination "$envProgramData\Tuolumne County\Application Settings\" -Recurse
        }
        # If copy failed, log results. Exit script.
        Catch [System.Exception] {
        ...
        Write-Log -Message "Copying application settings to settings directory complete."
        ...
        Try {
            ...
            Set-ActiveSetup -StubExePath "$appSettingsDir\$keyName.EXE" -Description "$keyName" -Key "$keyName"
        }
        # If setting up activesetup failed, exit script. Log results. 
        Catch [System.Exception] {
	...
	ElseIf ($deploymentType -ieq 'Uninstall') {
		...
		Write-Log -Message "Checking if previous versions of activesetup key exist." 
		If (Test-Path -Path "$keyPath") {
			Write-Log -Message "Previous versions of activesetup key exist." 
			# Remove previous versions of activesetup key.    
			Try {
				Write-Log -Message "Removing previous versions of activesetup key."
				Set-ActiveSetup -Key "$keyName" -PurgeActiveSetupKey
			...
			Write-Log -Message "Removing previous versions of activesetup key complete."
		}
		# Else, log results from check. 
		Else {  
			Write-Log -Message "Previous versions of activesetup key do not exist."        
